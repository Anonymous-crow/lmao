...
"""
Copyright (c) 2019 Valentin B. simple modification by hakkermanne
    ...
    async def _playlist(self, ctx: commands.Context, *, search: str):
        ...
        if "entries" in info_dict:
            for i in info_dict["entries"]:
                try:
                    source = await YTDLSource.create_source(ctx, i["webpage_url"], loop=self.bot.loop)
                except YTDLError as e:
                    await ctx.send('An error occurred while processing this request: {}'.format(str(e)))
                else:
                    song = Song(source)
